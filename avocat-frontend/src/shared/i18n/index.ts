...
export const t = (key: string, params?: TranslateOptions) => {
  ...
  const resolved =
    getNestedValue(primaryTable, key) ?? getNestedValue(fallbackTable, key);

  const resolvedValues =
    params && typeof params === 'object' && 'values' in params
      ? params.values
      : params;
  const values =
    resolvedValues && typeof resolvedValues === 'object'
      ...
      : undefined;
  const fallback =
    params && typeof params === 'object' && 'fallback' in params
      ? params.fallback
      : undefined;
