...
import { ArrowRight, BadgeCheck, Gavel, Landmark, Scale, ShieldCheck, Sparkles, Star } from "lucide-react";
    ...
    const content = {
      en: {
        ...
        services: {
          ...
          subtitle: "Strategic legal guidance across every stage of your business and personal matters.",
          items: [
            {
              title: "Corporate Advisory",
              description: "Executive-level counsel for governance, compliance, and transactions.",
              icon: Landmark,
            },
            {
              title: "Litigation Management",
              description: "Precision-driven litigation strategies with transparent reporting.",
              icon: Gavel,
            },
            {
              title: "Private Wealth",
              description: "Estate planning and asset protection with full confidentiality.",
              icon: Scale,
            },
          ],
        },
        why: {
          title: "Why Avocat",
          subtitle: "An elevated legal partnership designed for high-stakes decisions.",
          highlights: [
            {
              title: "Secure-by-design",
              description: "Gold-standard security architecture protects every document and conversation.",
              icon: ShieldCheck,
            },
            {
              title: "White-glove service",
              description: "Dedicated legal concierge ensuring swift responses and strategic clarity.",
              icon: BadgeCheck,
            },
            {
              title: "Premium insights",
              description: "Data-informed reporting keeps you ahead of every legal milestone.",
              icon: Star,
            },
          ],
        },
        cta: {
          ...
          description: "Join the Avocat platform and connect with your legal team in minutes.",
          button: "Start now",
        },
        footer: {
          title: "Avocat Legal",
      ...
      ar: {
        ...
        services: {
          ...
          subtitle: "إرشاد قانوني استراتيجي في جميع مراحل أعمالك وشؤونك الخاصة.",
          items: [
            {
        ...
        why: {
          ...
          highlights: [
            ...
            {
              title: "خدمة نخبوية",
              description: "مستشار قانوني مخصص يضمن سرعة الاستجابة ووضوح الرؤية.",
              icon: BadgeCheck,
            },
            {
              title: "رؤى متقدمة",
        ...
        cta: {
          ...
          description: "انضم إلى منصة أفوكات وتواصل مع فريقك القانوني خلال دقائق.",
          button: "ابدأ الآن",
        },
        footer: {
    ...
    <div className="min-h-screen bg-background text-foreground">
      <section className="relative overflow-hidden bg-gradient-hero text-white">
        ...
        <div className="relative z-10">
          ...
          <div className="mx-auto grid w-full max-w-6xl grid-cols-1 gap-10 px-6 pb-20 pt-6 lg:grid-cols-2 lg:items-center">
            <motion.div initial="hidden" animate="visible" variants={motionVariants} className="space-y-6">
              <div className={cn("inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs uppercase tracking-[0.2em]", isRTL && "flex-row-reverse")}> 
                <Sparkles className="h-4 w-4" />
                <span>{copy.hero.eyebrow}</span>
              </div>
              <h1 className={cn("text-4xl font-semibold leading-tight md:text-5xl", isRTL ? "font-arabic" : "font-display")}> 
                {copy.hero.title}
              </h1>
              <p className="text-base text-white/80 md:text-lg">{copy.hero.description}</p>
              <div className={cn("flex flex-col gap-3 sm:flex-row", isRTL && "sm:flex-row-reverse")}> 
                <Button size="lg" variant="premium" onClick={() => navigate("/login")}
              ...
              <div className="grid grid-cols-3 gap-4 pt-4 text-center text-sm">
                {copy.hero.stats.map((stat) => (
                  <div key={stat.label} className="rounded-xl border border-white/15 bg-white/5 px-3 py-4">
                    <p className="text-lg font-semibold text-[hsl(var(--gold))]">{stat.value}</p>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-16">
        <div className="flex flex-col gap-6 rounded-[var(--radius-xl)] border border-border bg-[hsl(var(--card))] p-8 shadow-custom-lg md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-muted-foreground">{copy.trust.title}</p>
          </div>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-16">
        <div className="mb-10 flex flex-col gap-3">
          <h2 className={cn("text-3xl font-semibold", isRTL ? "font-arabic" : "font-display")}>{copy.services.title}</h2>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-16">
        <div className="mb-10 flex flex-col gap-3">
          <h2 className={cn("text-3xl font-semibold", isRTL ? "font-arabic" : "font-display")}>{copy.why.title}</h2>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {copy.why.highlights.map((item) => {
            const Icon = item.icon;
            return (
              <div key={item.title} className="rounded-[var(--radius-xl)] border border-border bg-[hsl(var(--surface))] p-6 shadow-custom-md">
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-20">
        <div className="rounded-[var(--radius-2xl)] bg-gradient-gold p-10 text-[hsl(var(--foreground))] shadow-gold">
          <div className={cn("flex flex-col items-start gap-6 md:flex-row md:items-center md:justify-between", isRTL && "md:flex-row-reverse")}> 
            <div>
              <h3 className={cn("text-2xl font-semibold", isRTL ? "font-arabic" : "font-display")}>{copy.cta.title}</h3>
              <p className="mt-2 text-sm text-[hsl(var(--foreground))]/80">{copy.cta.description}</p>
            </div>
      ...
      <footer className="border-t border-border bg-[hsl(var(--surface))]">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-6 py-10 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{copy.footer.description}</p>
          </div>
