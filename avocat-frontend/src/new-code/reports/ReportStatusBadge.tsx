...
const statusMap: Record<string, { en: string; ar: string; className: string }> = {
  ...
  active: { en: 'Active', ar: 'نشط', className: 'status-open' },
  inactive: { en: 'Inactive', ar: 'غير نشط', className: 'status-closed' },
  'on-hold': { en: 'On Hold', ar: 'معلقة', className: 'status-pending' },
};

const ReportStatusBadge = ({ status, lang = 'en' }: ReportStatusBadgeProps) => {
